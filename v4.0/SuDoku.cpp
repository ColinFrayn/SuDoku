...
// From the DL Solver
extern int g_History[82], g_lastDiff;
    ...
    case WM_COMMAND:
      ...
      switch (wmId) {
        ...
        case IDM_FILE_EXPORTHTML:
          ...
          break;
        case IDM_HELP_ABOUT:
          MessageBox(hWndMain,"SuDoku Solver v4.0 by Colin Frayn, 2005\ncolin@frayn.net\nhttp://www.frayn.net","About SuDoku Solver v4.0",MB_OK);
          break;
        case IDM_EDIT_RATE:
          n = RateBoard();
          if (n == D_EASY) sprintf(strTemp,"Rating : Easy  [%d]",g_lastDiff);
          if (n == D_MILD) sprintf(strTemp,"Rating : Mild  [%d]",g_lastDiff);
          if (n == D_MEDIUM) sprintf(strTemp,"Rating : Medium  [%d]",g_lastDiff);
          if (n == D_HARD) sprintf(strTemp,"Rating : Hard  [%d]",g_lastDiff);
          if (n == D_FIENDISH) sprintf(strTemp,"Rating : Fiendish  [%d]",g_lastDiff);
          if (n == D_NIGHTMARE) sprintf(strTemp,"Rating : Nightmare  [%d]",g_lastDiff);
...
void SetupMainWindow(void) {
  ...
  hWndMain = CreateWindow(szWindow,"Col's SuDoku Solver v4.0    (c) 2005 Colin Frayn   colin@frayn.net", WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS,
