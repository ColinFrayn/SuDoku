...
int g_lastx, g_lasty, g_iDiff, g_lastDiff, g_expCount;
// Scores for using various techniques
int MoveScore[12] = {1,4,4,5,3,10,10,15,20,25,30,20}, g_iAbort, g_SolutionCount, g_History[82];
int FirstTime[8] = {5,8,8,8,12,16,50,100};
...
#define REQUIRES_COMPLEX (10)
#define CANNOT_SOLVE     (1000)

// Rate the difficulty of the current board
int RateBoard(void) {
  int diff=0,x,y,n,Backup[9][9],done=81,turn,easy;
  bool BackupSet[9][9],BackupSolved[9][9], BackupPoss[9][9][9], bUsed[8]={false,false,false,false,false,false,false,false};
  ...
  easy = SimpleSolver(&diff);
  done -= easy;

  // If we didn't get many easy cells then this is a hard puzzle
  if (easy < 20) diff += (20-easy);
  if (done > 40) diff += (done-40);

  // If we haven't solved it with simple strategies alone then let's use something more complicated
  if (done>0) {
    diff += REQUIRES_COMPLEX; // Difficulty bonus for not being solvable using slicing
    do {
      turn=0;
      g_expCount=0;
      ConstrictionStep(true);
      RowColumnConstriction(true);  
      if (g_expCount > 0) diff += MoveScore[4];
      turn = SimpleSolver(&diff);
      done -= turn;
      if (turn>0) {
      ...
      g_expCount=0;
      UniqueSubsets(true);
      if (g_expCount > 0) diff += MoveScore[5];
      turn = SimpleSolver(&diff);
      done -= turn;
      if (turn>0) {
        for (y=0;y<9;y++) for (x=0;x<9;x++) if (Board[x][y] != 0 && !Solved[x][y]) SetSquare(x,y);
      ...
      if (g_expCount > 0) diff += MoveScore[6];
      turn = SimpleSolver(&diff);
      done -= turn;
      if (turn>0) {
        for (y=0;y<9;y++) for (x=0;x<9;x++) if (Board[x][y] != 0 && !Solved[x][y]) SetSquare(x,y);
      ...
      g_expCount=0;
      NeighbourPairs(true);
      if (g_expCount > 0) diff += MoveScore[7];
      turn = SimpleSolver(&diff);
      done -= turn;
      if (turn>0) {
        for (y=0;y<9;y++) for (x=0;x<9;x++) if (Board[x][y] != 0 && !Solved[x][y]) SetSquare(x,y);
      ...
      XWingRB(true);
      XWingCB(true);
      if (g_expCount > 0) diff += MoveScore[8] + g_expCount;
      turn = SimpleSolver(&diff);
      done -= turn;
      if (turn>0) {
        for (y=0;y<9;y++) for (x=0;x<9;x++) if (Board[x][y] != 0 && !Solved[x][y]) SetSquare(x,y);
      ...
      g_expCount=0;
      Swordfish(true);
      if (g_expCount > 0) diff += MoveScore[9] + g_expCount*2;
      turn = SimpleSolver(&diff);
      done -= turn;
      if (turn>0) {
        for (y=0;y<9;y++) for (x=0;x<9;x++) if (Board[x][y] != 0 && !Solved[x][y]) SetSquare(x,y);
      ...
      g_expCount=0;
      ForcingChains(true);
      if (g_expCount > 0) diff += MoveScore[10];
      turn = SimpleSolver(&diff);
      done -= turn;
      if (turn>0) {
        for (y=0;y<9;y++) for (x=0;x<9;x++) if (Board[x][y] != 0 && !Solved[x][y]) SetSquare(x,y);
      ...
      g_expCount=0;
      Nishio(true);
      if (g_expCount > 0) diff += MoveScore[11] + g_expCount;
      turn = SimpleSolver(&diff);
      done -= turn;
      if (turn>0) {
        for (y=0;y<9;y++) for (x=0;x<9;x++) if (Board[x][y] != 0 && !Solved[x][y]) SetSquare(x,y);
  ...
  for (n=0;n<8;n++) if (bUsed[n]) diff += FirstTime[n];

  // Penalty if this puzzle appears unsolvable
  if (done > 0) diff += CANNOT_SOLVE;
  ...
  if (diff<70)  return D_EASY;
  if (diff<100) return D_MILD;
  if (diff<130) return D_MEDIUM;
  if (diff<160) return D_HARD;
  if (diff<220) return D_FIENDISH;
  return D_NIGHTMARE;
}

// Do simple solving steps, if possible
int SimpleSolver(int *diff) {
  int x,y,turn,done=0;
